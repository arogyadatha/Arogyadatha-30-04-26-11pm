...
const features = [
  ...
  { icon: CalendarCheck, title: "Book appointment & History", color: "text-green-600", bg: "bg-green-50" },
  ...
  { icon: Leaf, title: "Functional Nutrition", color: "text-emerald-600", bg: "bg-emerald-50" },
  ...
  { icon: Stethoscope, title: "24/7 Jr. Doctors", color: "text-[#059669]", bg: "bg-emerald-50" },
    ...
    <section id="services" className="py-12 bg-white border-t border-gray-100 overflow-hidden">
      <div className="container mx-auto px-2 md:px-4">
        <div className="text-left mb-8 animate-in fade-in slide-in-from-left-4 duration-700">
          <h2 className="text-xl md:text-2xl font-black text-[#059669] uppercase tracking-[0.1em] border-l-4 border-[#1FD73D] pl-3">Our Services</h2>
        </div>

        <div className="border-2 border-[#059669]/10 rounded-3xl p-2 md:p-10 bg-gray-50/30 shadow-inner backdrop-blur-sm">
          <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-1.5 md:gap-8">
            {features.map((feature, i) => (
              <div 
                key={i} 
                className="flex flex-col items-center text-center p-1.5 md:p-6 rounded-xl md:rounded-3xl bg-white border border-[#059669]/5 hover:border-[#1FD73D]/40 hover:shadow-[0_20px_40px_-10px_rgba(31,215,61,0.15)] transition-all duration-300 cursor-pointer group hover:-translate-y-2 active:scale-95 animate-in fade-in zoom-in-95 duration-500"
              ...
              >
                ...
                <span className="text-[9px] md:text-xl font-bold text-[#111111] leading-tight uppercase tracking-tight h-8 md:h-16 flex items-center justify-center line-clamp-2 transition-colors group-hover:text-[#059669]">
                  {feature.title}
                </span>
              </div>
            ))}
